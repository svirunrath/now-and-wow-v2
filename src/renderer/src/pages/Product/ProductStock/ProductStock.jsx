...
const ProductStock = () => {
  ...
  if (stockItemData && !stockItemIsLoading) {
    for (let i = 0; i < stockItemData.stock_items.length; i++) {
      let total_sell_price = Number(stockItemData.stock_items[i].total_sell_price)
      ...
      stockDataSource.push({
        ...
        stock_id: stockItemData.stock_items[i].stock_id,
  ...
  } = useGetStockDetailByStockIdQuery({ stock_id: stockId }, { skip: stockId == undefined })
    ...
    if (record.key == expandedKey) {
      ...
      if (!stockDetailIsLoading && stockDetailData) {
        ...
        for (let i = 0; i < stock_details.length; i++) {
          ...
          let total_product_price = Number(stock_details[i].total_product_price)
          let product_unit_total_price = Number(stock_details[i].product_unit_total_price)
          stockDetailDataSource.push({
            key: i + 1,
            product_id: stock_details[i].product_id,
            import_date: stock_details[i].import_date,
            product_name: stock_details[i].product_name,
            product_qty: stock_details[i].product_qty,
            price_id: stock_details[i].price_id,
            product_unit_total_price: '$ ' + product_unit_total_price.toFixed(2),
